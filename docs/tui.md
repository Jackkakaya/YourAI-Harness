...
界面以对话和修改结果为主。顶部不设标题栏，直接展示对话；Ctrl-Home 回到最新提问，Ctrl-↑/↓ 浏览前后提问，Ctrl-End 跟随最新输出。用户提问以独立色块、加粗和留白区分，不显示 YOU / AGENT 标签；工具卡片展示命令结果、退出状态和文件 diff，未知工具展开后保留结构化字段值。输入区横跨窗口，不受 Todo 面板宽度影响，支持多行和 Unicode，仅保留底色和 › 提示符，不画外框或 Message 标题。默认五行（含上下留白，可显示三行输入），短窗口自动缩小，随内容自动增高，最多占窗口三分之一且不超过十行；长草稿随光标滚动。忙碌时其上方显示当前动作、耗时和 Esc 停止提示。滚动条隐藏，滚轮和键盘滚动仍可用。Agent 正文、工具摘要与思考折叠分别显示。
...
交互优化：输入区下方的 `↑ Question` 可点击回到问题，回看时 `↓ Latest` 返回最新输出。连续成功的读取/搜索操作以 `Explored` 摘要收起，点击或选中后 Ctrl-O 展开；失败和代码修改保持单独可见。

测试滚动手感建议使用优化构建：`cargo run --release -p yourai-tui -- --config ./yourai.json`。调试构建的渲染开销更大；基准及已知边界见 [TUI 性能记录](tui-performance.md)。

每轮的 `Recorded actions` 汇集已记录的失败/停止、文件操作和命令结果。点击具体条目可展开并定位原始 diff/日志，`/results` 可跳到最近结果区。每组预览最多三条；更多记录和完整详情仍可从原始工具条目检查。`exit 0` 只代表命令退出成功，不代表所有验证均通过。

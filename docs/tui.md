...
界面以对话和修改结果为主。顶部不设标题栏，直接展示对话；Ctrl-Home 回到最新提问，Ctrl-↑/↓ 浏览前后提问，Ctrl-End 跟随最新输出。用户提问以独立色块、加粗和留白区分，不显示 YOU / AGENT 标签；工具卡片展示命令结果、退出状态和文件 diff，未知工具展开后保留结构化字段值。输入区横跨窗口，不受 Todo 面板宽度影响，支持多行和 Unicode，仅保留底色和 › 提示符，不画外框或 Message 标题。默认三行（上下留白各一行，单行输入），短窗口自动缩小，随内容自动增高，最多占窗口三分之一且不超过八行；长草稿随光标滚动。忙碌时其上方显示当前动作、耗时和 Esc 停止提示。滚动条隐藏，滚轮和键盘滚动仍可用；滚轮每格滚动三行（与 codex/opencode 一致），mux/直连下自动关闭 2026 同步输出包裹以走复用器增量协议。Agent 正文、工具摘要与思考折叠分别显示。

底栏始终一行，横跨窗口。正常宽度显示标题、目录、累计 token、ctx、最近一次回复速度及权限；窄屏缩略标题和路径，优先保留 ctx 与权限，放不下的次要指标用 … 示意，Ctrl-B 查看模型及完整统计。速度未返回时显示 —。右侧仅在有 Todo 且宽度 ≥80 时出现任务面板，Ctrl-T 收起；没有任务时对话全宽。`/theme dark` 是纯黑背景配冰蓝强调，`/theme light` 是暖白配墨绿强调。
...
交互优化：Ctrl-Home 回到最新问题，回看时输入区显示简短的 `↓ Latest` 返回最新输出。连续成功的读取/搜索操作以 `Read & search` 摘要收起，点击或选中后 Ctrl-O 展开；失败和代码修改保持单独可见。

测试滚动手感建议使用优化构建：`cargo run --release -p yourai-tui -- --config ./yourai.json`。调试构建的渲染开销更大；基准及已知边界见 [TUI 性能记录](tui-performance.md)。
...
工具过程保持轻量：已完成操作使用常规字重，当前操作和失败保留重点。注释、运算符采用中性色，语法颜色集中在关键字、类型和字面量；工具 diff 与日志直接在原始条目展开。

分区布局：命令、文件修改及失败以无边框内容块显示，实际命令/路径在标题条，日志/diff 在下方；成功读取与搜索仍为轻量行。Todo 使用独立侧栏底色和留白，窗口不足 100 列时收为任务 dock。对话正文和紧凑输入区保持独立。

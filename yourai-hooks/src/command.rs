...
pub struct CommandHandler {
    command: String,
    shell: Option<crate::config::HookShell>,
    background: Option<BackgroundCommandContext>,
}

impl CommandHandler {
    pub fn new(command: String, shell: Option<crate::config::HookShell>) -> Self {
        Self {
            command,
            shell,
            background: None,
    ...
    async fn spawn(
        ...
        let mut cmd = match self.shell {
            Some(crate::config::HookShell::Powershell) => {
            ...
            Some(crate::config::HookShell::Bash) => {
    ...
    async fn run_with_async_detection(
        ...
        let async_response = match crate::wire_output::parse_hook_json(first_line.trim()) {
            Ok(crate::wire_output::HookJsonOutput::Async(output)) => Some(output),
            _ => None,
        };
...
impl HookHandler for CommandHandler {
    ...
    ) -> crate::handler::BoxFuture<'a, Result<HookOutput, yourai_core::YourAiError>> {
        Box::pin(async move {

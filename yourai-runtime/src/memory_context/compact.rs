...
impl MemoryContext {
    ...
    pub(super) async fn maintain(
        ...
        let before = self.estimate(
            &self.project(&records, options.system.as_deref(), &options.tools)?,
            execution,
        )?;
                            ...
                            && view.consumed.contains(&results[0].call_id)
                        {
                            ...
                            if value.get("ok").and_then(|v| v.as_bool()) == Some(true)
                                ...
                                && value.get("preserve_context").and_then(|v| v.as_bool())
                                    != Some(true)
                            {
                                m.tool_output_pruned_at = Some(crate::sqlite::now());
        ...
        let after_prune = self.estimate_raw(
            &self.project(&projected, options.system.as_deref(), &options.tools)?,
            execution,
        )?;
        ...
        let after = self.estimate_raw(
            &self.project(&retained, options.system.as_deref(), &options.tools)?,
            execution,
        )?;

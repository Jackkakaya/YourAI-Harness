...
#[tokio::test]
async fn schema_contains_only_three_tables_and_no_source_identity_column() {
    let dir = TempDir::new().unwrap();
    let _store = open(&dir);
    let db = rusqlite::Connection::open(SqliteStore::path(dir.path())).unwrap();
    let mut q = db
    ...
    let tables = q
        ...
        .unwrap();
    assert_eq!(tables, vec!["messages", "sessions", "usage_events"]);
    let ddl: String = db
        .query_row(
...
async fn legacy_import_preserves_sources_and_is_transactional() {
    ...
    .unwrap();
    assert!(yourai_runtime::SessionCatalog::new(dir.path()).is_err());
    assert_eq!(
        yourai_runtime::sqlite::import_json_sessions(dir.path()).unwrap(),
        1
    );
    assert_eq!(
    ...
    let catalog = yourai_runtime::SessionCatalog::new(dir.path()).unwrap();
    ...
    assert!(yourai_runtime::sqlite::import_json_sessions(dir.path()).is_err());
    assert_eq!(
        catalog
            .read_messages(&id, MessageQuery::default())
            .await
...
async fn version_one_migrates_pruning_column_without_changing_original_messages() {
    ...
    db.execute_batch(
        "ALTER TABLE messages DROP COLUMN tool_output_pruned_at; PRAGMA user_version=1;",
    )
    .unwrap();
    drop(db);
    let store = SqliteStore::open(&path).unwrap();

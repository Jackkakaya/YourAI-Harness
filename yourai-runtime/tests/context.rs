...
use yourai_runtime::{local::LocalUsage, SqliteStore};
...
async fn projection_is_bounded_valid_json_and_original_is_pageable() {
    ...
    .await;
    let projected = c.build_request(Some("system"), &[]).unwrap();
    ...
    let reader = yourai_runtime::tool_result::ReadToolResult {
...
async fn prune_only_avoids_model_hooks_and_survives_restore_and_fork() {
    ...
    let expected = serde_json::to_value(c.build_request(None, &[]).unwrap().request).unwrap();
    c.restore().await.unwrap();
    assert_eq!(
        expected,
        serde_json::to_value(c.build_request(None, &[]).unwrap().request).unwrap()
    );
    ...
    assert_eq!(
        expected,
        serde_json::to_value(fork.build_request(None, &[]).unwrap().request).unwrap()
    );
}
#[tokio::test]
async fn failed_combined_commit_keeps_pruning_and_summary_unapplied_but_accounts_usage() {
    ...
    assert!(c
        ...
        .await
        .is_err());
    assert!(c.build_request(None, &[]).is_err()); // cannot issue requests with uncertain memory
    c.restore().await.unwrap();
...
#[tokio::test]
async fn observed_usage_is_invalidated_by_tools_system_or_pruning_projection() {
    let (_, _, c) = setup(policy(), Summarizer::new(), None).await;
    append(&c, vec![ChatMessage::user("x".repeat(2000))]).await;
    let before = c.build_request(Some("sys"), &[]).unwrap();
    ...
    let calibrated = c.build_request(Some("sys"), &[]).unwrap().estimated_tokens;
    let different = c
        .build_request(Some("changed"), &[])
        .unwrap()
        .estimated_tokens;
    assert!(calibrated < 200);
    assert!(different > 600);
    assert!(
        c.build_request(Some("sys"), &[Tool::new("different")])
            .unwrap()
            .estimated_tokens
            > 600
    );
...
async fn automatic_cooldown_and_unknown_window_do_not_claim_success() {
    ...
    .await;
    assert_eq!(c.build_request(None, &[]).unwrap().input_budget, None);
    assert!(c
        .compact(manual(), &CancellationToken::new())
        .await
        .is_err());
...
async fn automatic_failure_is_not_repeated_for_the_same_request() {
    ...
    .await;
    assert!(c.build_request(None, &[]).unwrap().maintenance_needed);
    assert!(c
        .compact(
            CompactionRequest::new(CompactionTrigger::Threshold),
            &CancellationToken::new()
...
async fn cancelled_uncertain_commit_blocks_reads_and_next_append_recovers_first() {
    ...
    assert!(c.records()[0].summary);
    assert_eq!(c.records().len(), 3);
    assert!(c.build_request(None, &[]).is_ok());
}

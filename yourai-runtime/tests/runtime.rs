...
fn context(
    ...
    services.usage = Some(Arc::new(local::LocalUsage((*store).clone())));
...
async fn real_compaction_commits_summary_but_retains_transcript_and_identities() {
    ...
    assert_eq!(
        history
            .build_request(Some("instructions"), &[])
            .unwrap()
            .request
            .system
            .as_deref(),
...
async fn async_hook_completion_routes_to_own_session_and_wakes_host() {
    let dir = TempDir::new().unwrap();
    let runtime = Arc::new(yourai_hooks::ConcreteHookRuntime::new());
    let config:yourai_hooks::HooksConfig=serde_json::from_value(json!({"hooks":{"SessionStart":[{"hooks":[{"type":"command","command":"echo background-feedback >&2; exit 2","asyncRewake":true}]}]}})).unwrap();
    runtime
...
async fn harness_runs_real_task_tool_through_loop_and_restores_session() {
    ...
    resumed
        ...
        .unwrap();
    assert!(
        format!("{:?}", resumed_model.requests.lock().unwrap()[0].request)
...
async fn agent_hook_uses_real_loop_and_shared_budget() {
    use yourai_hooks::{HookModelExecutor, HookModelRequest};
    let budget = ModelBudget::new(Some(1), None);
    let executor = model_hooks::DefaultHookModelExecutor {
...
async fn closing_session_cancels_its_background_command() {
    ...
    let runtime = Arc::new(yourai_hooks::ConcreteHookRuntime::new());
    let config:yourai_hooks::HooksConfig=serde_json::from_value(json!({"hooks":{"SessionStart":[{"hooks":[{"type":"command","command":format!("sleep 0.3; touch '{}'",marker.display()),"async":true}]}]}})).unwrap();
    runtime
        .register_config(&config, HookSource::Session)
        .await
        .unwrap();
...
async fn shared_hook_runtime_keeps_background_contexts_session_local() {
    let dir = TempDir::new().unwrap();
    let runtime = Arc::new(yourai_hooks::ConcreteHookRuntime::new());
    let config:yourai_hooks::HooksConfig=serde_json::from_value(json!({"hooks":{"SessionStart":[{"hooks":[{"type":"command","command":"sleep 0.1; echo feedback >&2; exit 2","asyncRewake":true}]}]}})).unwrap();
    runtime
        .register_config(&config, HookSource::Session)
        .await
        .unwrap();
...
#[tokio::test]
async fn basic_harness_leaves_extensions_uninstalled() {
    let dir = TempDir::new().unwrap();
    let h = Harness::open(
...
#[tokio::test]
async fn manual_compact_uses_current_execution_providers_and_loop_system() {
    ...
    let usage = Arc::new(local::LocalUsage((*store).clone()));
    agent.ctx().set_usage(usage.clone());
    let system = "current system ".repeat(40);
    agent
        .ctx()
        .set_agent_loop(Arc::new(yourai_loop::DefaultLoop::new(
    ...
    let before = history
        .inner
        .build_request(Some(&system), &[], &execution)
        .unwrap()
        .estimated_tokens;
    let result = h
        .compact(
